...
public class AStar : MonoBehaviour {
	...
	IEnumerator FindPath(){
		...
		while(openList.Count>0){
			...
			foreach(Node neighbor in neighbors){
				if(closedList.Contains(neighbor)) {	// Ignore the neighbor which is already evaluated.
					continue;
				}
				if(!openList.Contains(neighbor)){
					...
					if(tentative_gScore>=neighbor.gScore){	 // This is not a better path.
						continue;
					}
	...
	//TODO: Find out why the heck the path takes obstacle nodes
